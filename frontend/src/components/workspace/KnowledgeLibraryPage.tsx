...
export default function KnowledgeLibraryPage() {
  ...
  const inspect = async (id: string) => {
    setMessage('');
    try { setSelected(await api(`/documents/${id}`) as DocumentDetail); }
  ...
  const indexDocument = async (document: DocumentSummary) => {
    setIndexing(document.id); setMessage('');
    try {
  ...
  return <div className="workspace-page library-page">
    ...
    <section className="library-metrics" aria-label="Knowledge library overview">
      <article><span><Library size={18} /></span><div><strong>{documents.busy ? '—' : list.length}</strong><small>Stored documents</small></div></article>
      <article><span><Database size={18} /></span><div><strong>{status.busy ? '—' : typeof chunks === 'number' ? chunks : 'Unavailable'}</strong><small>Indexed chunks</small></div></article>
      <article><span><BookOpen size={18} /></span><div><strong>{documents.busy ? '—' : indexed}</strong><small>RAG-indexed files</small></div></article>
      <article><span><FileSearch size={18} /></span><div><strong>{documents.busy ? '—' : verified}</strong><small>Source-verified records</small></div></article>
    </section>
