...
export default function ReportsPage() {
  ...
  useEffect(() => { setSceneMetrics({ nodeCount: reports.length }); }, [reports.length]);

  const open = async (id: string) => {
    setOpening(id); setMessage(''); setScenePhase('processing');
    try { setSelected(await api(`/reports/${id}`)); setScenePhase('success', 'report-opened'); }
    catch (reason) { setMessage(reason instanceof Error ? reason.message : 'The report could not be opened.'); setScenePhase('error', 'report-error'); }
    finally { setOpening(''); }
  };

  const exportReport = (format: string, suffix: string) => {
    if (!selected) return;
    pulseScene('report-export');
