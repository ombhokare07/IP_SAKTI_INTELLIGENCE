...
test('fixture labels cannot look like live search',()=>{assert.equal(describeMode('mock'),'Synthetic test data');assert.equal(describeMode('synthetic'),'Synthetic test data');assert.equal(describeMode('grounded'),'Grounded evidence');assert.equal(describeMode('live'),'Live provider mode');assert.equal(describeMode('unconfigured'),'No provider configured');});
